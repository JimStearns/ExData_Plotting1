...
doThePlot <- function(df) {
    par(mar=c(5.1,4.1,3.1,2.1), mfrow=c(2,2), cex.axis=1)
    
    # Upper-left: Global active power (plot2)
    with(df, plot(DateTime, Global_active_power, type="n", 
                  xlab="", ylab="Global Active Power"))
    with (df, lines(DateTime, Global_active_power, lwd=1))
    
    # Upper-right: voltage over time (new)
    # Don't plot the lines, and don't label the y axis ticks
    with(df, plot(DateTime, Voltage, type="n", xlab="datetime", yaxt="n"))
    # Add the lines ...
    with(df, lines(DateTime, Voltage))
    # And the y-axis ticks with labels
    axis(2, cex.axis=0.9)
    
    # Lower-left: energy sub-metering (plot3)
    with(df, plot(DateTime, Sub_metering_1, type="n", 
         xlab="", ylab="Energy sub metering"))
    with (df, lines(DateTime, Sub_metering_1, lwd=1, col="black"))
    with (df, lines(DateTime, Sub_metering_2, lwd=1, col="red"))
    with (df, lines(DateTime, Sub_metering_3, lwd=1, col="blue"))
    # Shrink the legend a bit, and remove the outline.
    legend("topright", lty=1, col = c("black", "red", "blue"), bty="n", cex=0.9,
        legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
    
    # Lower-right: global reactive power (new)
